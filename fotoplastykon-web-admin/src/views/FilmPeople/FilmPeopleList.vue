            ...
            >
                <template v-slot:top>
                    <v-toolbar flat color="white">
                        ...
                        <v-btn color="primary" dark class="mb-2" @click="$router.push({ name: 'film-person-add' })">Dodaj</v-btn>
                    </v-toolbar>
                </template>
...
<script lang="ts">
    ...
    export default class FilmPeopleListComponent extends Vue {
        ...
        async editItem(id: number) {
            await this.$router.push({ name: 'film-person-edit', params: {id: id.toString()} });
        }

        async deleteItem(id: number) {
            await FilmPeopleService.delete(id);
            await this.loadData();
        }
    }
</script>
