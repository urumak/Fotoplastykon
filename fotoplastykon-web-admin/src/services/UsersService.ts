...
export default class UsersService {
    ...
    public static async add(model: UserFromModel): Promise<number> {
        return (await Vue.axios.post<number>(`admin/users/add`, model)).data;
    }
    ...
    public static async changeProfilePhoto(id: number, file: any): Promise<any> {
        let form = new FormData();
        form.append('file', file);
        return (await Vue.axios.post<any>(`admin/users/change-profile-photo/${id}`, form)).data;
    }
}
