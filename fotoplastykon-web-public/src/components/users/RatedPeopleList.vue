<template>
    <div>
        <v-list>
            ...
            >
                <v-list-item-content>
                    <v-list-item-title>Brak ocenionych osób</v-list-item-title>
                </v-list-item-content>
            </v-list-item>
            <v-list-item
...
<script lang="ts">
    ...
    @Component({})
    export default class RatedPeopleList extends Vue {
        private people: RankModel[] = [];
        ...
        async loadData() {
            ...
            let response = await FilmPeopleService.getRatedPeople(this.pager, this.id);
