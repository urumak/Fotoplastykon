<template>
    <div>
        <v-list>
            <v-list-item
                    v-if="filmMakeres.length === 0"
            >
                <v-list-item-content>
                    <v-list-item-title>Brak twórców</v-list-item-title>
                </v-list-item-content>
            </v-list-item>
            <v-list-item
                    v-for="(item, index) in filmMakeres"
                    :key="index"
            ...
            >
                ...
                <v-list-item-content>
                    ...
                    <v-list-item-subtitle>{{ item.roleType }}</v-list-item-subtitle>
                </v-list-item-content>
            </v-list-item>
        </v-list>
...
<script lang="ts">
    ...
    export default class FilmMakers extends Vue {
        ...
        async loadData() {
            ...
            let response = await FilmsService.getFilmMakers(this.pager, this.id);
            this.filmMakeres = response.items;
